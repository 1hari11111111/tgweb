...
import { sendTelegramMessage, getBot } from '@/lib/bot'
      ...
      if (String(msg.chat.id) === settings.adminChatId && replyTo.text?.includes('Reply to this message with the custom INR amount to approve for Deposit #')) {
        ...
        if (depositIdMatch) {
          ...
          if (isNaN(customAmount) || customAmount <= 0) {
            const bot = getBot(); if (bot) await bot.sendMessage(msg.chat.id, "Invalid amount. Please reply with a valid number.").catch(console.error)
            return NextResponse.json({ ok: true })
          }
          ...
          if (transaction && transaction.status === 'PENDING') {
            ...
            const bot = getBot(); if (bot) await bot.sendMessage(msg.chat.id, `✅ Custom Deposit Approved for ₹${customAmount}`).catch(console.error)
